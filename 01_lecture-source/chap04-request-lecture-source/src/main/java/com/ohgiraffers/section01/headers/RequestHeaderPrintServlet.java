...
public class RequestHeaderPrintServlet extends HttpServlet {
    ...
    *   1. General Header - 요청과 응답 모두 적용되는 해더, 상태 메세지를 나타낸다.
    *   2. Request Header - 조회 될 리소스(정보)나 클라이언트 자체에 대한 상세 정보를 포함
    *   3. Response Header - 응답에 대한 부가적인 정보를 가지고 있는 헤더
    ...
    *   */
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        // 겟헤더 가지고 있는 모든 정보들을 가지고 와줌
        Enumeration<String> headerNames = req.getHeaderNames();
        // 다음요소들이 있니? 있으면 true, 없으면 false
        while (headerNames.hasMoreElements()){
            System.out.println(headerNames.nextElement());
        }

        // accept Header : 클라이언트에서 웹 서버로 request 할 시 요청할 수 있는 메세지에 담기는 헤더
        // 쉽게 설명하면, 나는 출력 된 값들만 accept 즉 수용할 수 있어 라는 것과 같음 -> json -> 나는 json 을 받을 꺼야
